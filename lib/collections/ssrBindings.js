...
// From collectionRead (NOT routeHelpers): this module is reached from the
// renderer, which is part of the page bundle, and routeHelpers imports `net`.
import { resolveCollection, projectPublicItem } from '@/lib/collections/collectionRead'
 ...
 * @param {Array<{collection, order, filter, limit}>} args.bindings from findCollectionBindings
...
export async function loadBoundCollections({ clientId, siteSlug, bindings }) {
  ...
  // One entry per DISTINCT (collection, order, filter, limit): the same list
  // bound twice on a page (say in the body and in the footer) is one query.
  ...
  for (const [key, binding] of wanted) {
    ...
    // A template asking for a filter it may not have is the same class of
    // authoring bug as a bad data-order: warn and drop the FILTER, rendering
    // the list unfiltered rather than blanking a client's page. Dropping the
    // whole filter (never applying it half-way) is why resolveFilters returns
    // an empty list alongside its error.
    const { filters, error: filterError } = resolveFilters({
      requested: binding.filter,
      allowedFields: collection.public_read_fields,
    })
    if (filterError) {
      console.warn(
        `[collections] binding "${binding.collection}" on site ${siteSlug} declared ` +
        `data-filter=${JSON.stringify(binding.filter)}, which is malformed or not in ` +
        'public_read_fields — rendering the list unfiltered'
      )
    }
