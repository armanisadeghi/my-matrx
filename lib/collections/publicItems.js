...
import { applyFilters } from '@/lib/collections/filtering'
 ...
 * @param {Array<{field, op, value}>} [args.filters] resolved filters (AND-ed)
 ...
 */
export async function fetchPublicItems(supabase, { collectionId, order, filters, from, to }) {
  const base = supabase
  ...
  // Filters narrow BEFORE the order and the range are applied, so `limit` means
  // "the first N matching rows", not "the first N rows, some of them hidden".
  const { data, error } = await applyOrder(applyFilters(base, filters), order).range(from, to)
