...
//
// RESOLUTION ORDER:
//   1. `client_sites.navigation` non-empty array of {label, href} → override.
//      Root-relative paths are site-relative and receive `basePath`; anchors,
//      absolute URLs, mailto: and tel: remain authored as-is.
...
/**
 * Resolve an authored href against the serving surface. A root-relative path
 * belongs to the client site, not to mymatrx.com itself: `/about` therefore
 * becomes `/c/{slug}/about` on the platform surface and stays `/about` on a
 * custom domain. Already-prefixed paths and protocol-relative URLs are left
 * untouched so renderer-owned and external links are never double-prefixed.
 */
export function resolveSiteHref(href, basePath = '') {
  const safe = safeHref(href)
  if (safe === '#' || !basePath || !safe.startsWith('/') || safe.startsWith('//')) return safe
  if (safe === basePath || safe.startsWith(`${basePath}/`)) return safe
  return `${basePath}${safe}`
}

/**
 * Rebase root-relative href attributes in agent-authored HTML. Script/style
 * bodies and comments are deliberately opaque: a string literal that happens
 * to contain `href="/x"` is not a link and must not be rewritten.
 */
export function rebaseSiteHrefs(html, basePath = '') {
  if (typeof html !== 'string' || !basePath || !html.includes('href')) return html
  return html.replace(
    ...
    (token) => {
      ...
      return token.replace(
        /(\bhref\s*=\s*)(["'])(\/(?!\/)[^"']*)\2/gi,
        (_match, prefix, quote, href) => `${prefix}${quote}${resolveSiteHref(href, basePath)}${quote}`
      )
    }
  )
}
...
export function resolveNavItems({ navigation, pages, basePath = '', pagePath }) {
  ...
  if (Array.isArray(navigation) && navigation.length > 0) {
    return navigation
      ...
      .map((entry) => ({ label: String(entry.label), href: resolveSiteHref(entry.href, basePath) }))
  }
  ...
  return candidates.map((page) => ({
    ...
    href: resolveSiteHref(`${basePath}${pagePath(page)}`, basePath),
  }))
}
