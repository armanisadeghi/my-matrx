 ...
 * @returns {Array<{collection: string, order: string|null, filter: string|null, limit: number|null}>}
...
export function findCollectionBindings(html) {
  ...
  for (const node of root.querySelectorAll('template[data-matrx-collection]')) {
    ...
    bindings.push(readBindingAttributes(node, collection))
  }
  return bindings
}

/**
 * Read one `<template>`'s binding attributes.
 *
 * ONE reader for both passes. The scan pass and the expand pass have to derive
 * byte-identical keys or the expander looks up rows that were never fetched and
 * silently renders every list empty; two copies of this parsing was that bug
 * waiting to happen, and it grew more likely with every attribute added.
 */
function readBindingAttributes(node, collection) {
  const rawOrder = node.getAttribute('data-order')
  const rawFilter = node.getAttribute('data-filter')
  const rawLimit = parseInt(node.getAttribute('data-limit'), 10)
  return {
    collection,
    order: typeof rawOrder === 'string' && rawOrder.trim() ? rawOrder.trim() : null,
    filter: typeof rawFilter === 'string' && rawFilter.trim() ? rawFilter.trim() : null,
...
export function bindingKey({ collection, order, filter, limit }) {
  return `${collection}|${order || ''}|${filter || ''}|${limit || ''}`
}
...
export function expandCollectionBindings(html, rowsByKey) {
  ...
  for (const node of templates) {
    ...
    const key = bindingKey(readBindingAttributes(node, collection))
    const rows = get(key)
