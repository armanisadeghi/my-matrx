...
/**
 * Serve the exact IndexNow ownership file from either site's catch-all route.
 * Returns null when the request is an ordinary content route.
 */
export function serveIndexNowKeyIfRequested({ client, slugSegments, res }) {
  if (!isIndexNowKeyRequest(slugSegments, client?.web_site_id)) return null
  const key = indexNowKeyForWebSiteId(client.web_site_id)
  return sendText(res, {
    contentType: 'text/plain; charset=utf-8',
    body: `${key}\n`,
  })
