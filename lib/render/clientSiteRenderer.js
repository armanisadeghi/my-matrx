...
import { hasNavToken, injectNav, rebaseSiteHrefs, resolveNavItems } from '@/lib/render/siteNav'
...
function componentHtml(component, derived, currentHref) {
  ...
  return rebaseSiteHrefs(injectFooter(withNav, derived.footerHtml), derived.basePath)
}
...
function renderListingPage(client, page, relatedPages, components, isPreview, nav, derived) {
  ...
  return (
    <>
      ...
      {/* Page content */}
      <div dangerouslySetInnerHTML={{ __html: rebaseSiteHrefs(page.html_content, nav.basePath) }} />
