...
export function ClientSiteRenderer({ client, page, relatedPages, components, isPreview, isList, notFound, nav, themeCss, navItems, footerHtml }) {
  if (notFound) {
    return (
      <>
        <Head>
...
export async function loadSitePageProps({ client, slugSegments, isPreview, nav, req, res }) {
  ...
  if (slug.length === 0) {
    ...
    // Deliberately still the bare slug, NOT pagePath(homePage): a live site root
    // already 302s here and the target must not move. A home page is root-level
    // by definition, so its slug alias and its route address the same page; the
    // legacy slug lookup below is what keeps that true.
