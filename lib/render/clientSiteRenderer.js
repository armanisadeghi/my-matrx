...
// Both are computed in loadSitePageProps (server-side) so warnings land in
...
export function ClientSiteRenderer({ client, page, relatedPages, components, isPreview, isList, notFound, nav, themeCss, navItems }) {
  if (notFound) {
    return (
      <>
        <Head>
  ...
  const derived = { themeCss: themeCss || '', navItems: navItems || [] }
...
/**
 * Header/footer html with the nav token resolved. Returns the row's html
 * UNCHANGED when there is no `<!--matrx:nav-->` in it.
 */
function componentHtml(component, navItems, currentHref) {
  return injectNav(component.html_content, navItems, currentHref)
}
...
function renderListingPage(client, page, relatedPages, components, isPreview, nav, derived) {
  ...
  return (
    <>
      ...
      {page.use_client_header && headerComponent && <div dangerouslySetInnerHTML={{ __html: componentHtml(headerComponent, derived.navItems, currentHref) }} />}
      ...
      {page.use_client_footer && footerComponent && <div dangerouslySetInnerHTML={{ __html: componentHtml(footerComponent, derived.navItems, currentHref) }} />}
    </>
  )
}
