...
function renderNormalPage(client, page, components, isPreview, nav) {
  ...
  return (
    <>
      ...
      <div dangerouslySetInnerHTML={{ __html: page.html_content }} />
      {page.use_client_footer && footerComponent && <div dangerouslySetInnerHTML={{ __html: footerComponent.html_content }} />}
      {usesMatrxData(page.js_content) && <script src="/matrx-data.js" />}
      {page.js_content && <script dangerouslySetInnerHTML={{ __html: page.js_content }} />}
    </>
  )
}

// ── W2-C: auto-include the MatrxData helper when the page's JS uses it ───────
...
// js_content IIFE — the events list sits on "Loading…" forever and the form's
// submit handler never binds. Making the agent remember an include is a
// footgun with no failure signal, so the platform includes it for them.
// A classic (non-deferred, non-module) <script src> before the inline script
// blocks until loaded, guaranteeing window.MatrxData exists when js_content
// runs. Emitted only when the page actually references it — pages that don't
// use collections pay nothing.
function usesMatrxData(jsContent) {
  return typeof jsContent === 'string' && jsContent.includes('MatrxData')
}
 ...
 */
export async function loadSitePageProps({ client, slugSegments, isPreview, nav, req }) {
