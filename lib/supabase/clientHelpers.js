...
/**
 * THE 301 LAW — one ledger lookup for a route that resolved no page.
 *
 * `client_redirects` (CMS migrations 0032-0034) holds one row per moved live
 * URL, UNIQUE on (client_id, from_route) and CHAIN-COLLAPSED on write, so a
 * single equality match is the whole resolution — never walk chains here.
 * Takes the client_id directly: the caller (loadSitePageProps) already holds
 * the resolved site row.
 *
 * @param {string} clientId - client_sites.id
 * @param {string} fromRoute - Full path with a leading slash
 * @returns {Promise<string|null>} The to_route, or null
 */
export async function getClientRedirect(clientId, fromRoute) {
  if (!clientId || !fromRoute) return null
  const supabase = getSupabaseClient()
  ...
  return data ? data.to_route : null
}
