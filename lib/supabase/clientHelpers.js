...
/**
 * Client fields the /c/ renderer actually reads (verified against the JSX in
 * pages/c/[client]/[[...slug]].js): slug (link hrefs + canonical URL), name
 * (title fallbacks), global_css (injected <style>), favicon (<link rel=icon>),
 * meta_defaults (description/og-image fallbacks).
 */
export function toRenderClientProps(client) {
  return pick(client, ['slug', 'name', 'global_css', 'favicon', 'meta_defaults'])
}
...
/**
 * Strip draft/unpublished content from a row before it reaches the browser.
 * Preview mode merges drafts into the live fields server-side (see
 * getClientPage/getClientComponents), so `*_draft` keys are never read by the
 * renderer — but left in place they serialize unpublished content into public
 * `__NEXT_DATA__`. Keeps `has_draft` / `_isPreview` flags (used by the preview
 * banner).
 */
